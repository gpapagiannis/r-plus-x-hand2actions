...
class Renderer:
    ...
    def __init__(self, cfg: CfgNode, faces: np.array, load_from_custom_file=False):
        """
    ...
    def __call__(self,
        ...
        # mesh_pc = pyrender.Mesh.from_points(mesh.vertices)

        # # Show point cloud using open3d
        # print('Showing point cloud=====================')
        # import open3d as o3d
        # pcd = o3d.geometry.PointCloud()
        # pcd.points = o3d.utility.Vector3dVector(mesh_pc.vertices)
        ...
        return output_img, rend_depth, mesh
    ...
    def render_rgba_multiple(
        ...
        trimesh_list = [self.vertices_to_trimesh(vvv, ttt.copy(), mesh_base_color, rot_axis, rot_angle, is_right=sss) for vvv,ttt,sss in zip(vertices, cam_t, is_right)]
        scene = pyrender.Scene(bg_color=[*scene_bg_color, 0.0],
                               ambient_light=(0.3, 0.3, 0.3))
        ...
        camera_center = [render_res[0] / 2., render_res[1] / 2.]
        focal_length = focal_length if focal_length is not None else self.focal_length
        print(f'[render_rgba_multiple] Focal length: {focal_length}')
        print(f'[render_rgba_multiple] Camera center: {camera_center}')
        camera = pyrender.IntrinsicsCamera(fx=focal_length, fy=focal_length,
                                           cx=camera_center[0], cy=camera_center[1], zfar=1e12)
        ...
        # rend_depth = rend_depth * 2.0 - 1.0
        # rend_depth = 2.0 * focal_length / (rend_depth * render_res[1])

        color = color.astype(np.float32) / 255.0
        renderer.delete()

        return color, rend_depth, trimesh_list
